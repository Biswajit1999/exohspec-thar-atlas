...
def _save_profiles(
    ...
    for ax, (label, values), color in zip(axes, illustrative, colors):
        ...
        ax.set_ylim(0, 1.12)
    wavelength_axis = axes[0].secondary_xaxis("top", functions=(lambda x: x, lambda x: x))
    wavelength_axis.set_xticks([0.0, 0.5, 1.0], [r"$\lambda_{min}$", r"$\lambda_{mid}$", r"$\lambda_{max}$"])
    wavelength_axis.set_xlabel("Wavelength parameter $\lambda(x,m)$ after calibration — values not yet solved")
